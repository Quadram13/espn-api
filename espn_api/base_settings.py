...
class BaseSettings(object):
    ...
    def __init__(self, data: Dict[str, Any]) -> None:
        ...
        self.playoff_matchup_period_length: int = data.get('scheduleSettings', {}).get('playoffMatchupPeriodLength', 0)
        self.playoff_seed_tie_rule: str = data['scheduleSettings']['playoffSeedingRule']
        self.scoring_type: Optional[str] = data.get('scoringSettings', {}).get('scoringType')
        self.median_scoring: bool = data.get('scoringSettings', {}).get('scoringEnhancementType') == 'WIN_BONUS_TOP_HALF'
        ...
        self.acquisition_budget: int = data.get('acquisitionSettings', {}).get('acquisitionBudget', 0)
        self.acquisition_limit: Optional[int] = data.get('acquisitionSettings', {}).get('acquisitionLimit')
        self.matchup_acquisition_limit: Optional[int] = data.get('acquisitionSettings', {}).get('matchupAcquisitionLimit')
        self.matchup_limit_per_scoring_period: Optional[bool] = data.get('acquisitionSettings', {}).get('matchupLimitPerScoringPeriod')
        self.minimum_bid: int = data.get('acquisitionSettings', {}).get('minimumBid', 0)
        self.waiver_process_days: List[str] = list(data.get('acquisitionSettings', {}).get('waiverProcessDays', []))
        self.waiver_process_hour: Optional[int] = data.get('acquisitionSettings', {}).get('waiverProcessHour')
        self.trade_revision_hours: Optional[int] = data.get('tradeSettings', {}).get('revisionHours')
